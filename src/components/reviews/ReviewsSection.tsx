...
export function ReviewsSection({ destinationId }: { destinationId: number }) {
  ...
  const processReviews = (rawReviews: Review[] = []) => {
    // For now, we will render a flat list but style it to support replies visually
    // since the backend Review object is flat
    return rawReviews
  }
        ...
        onSubmit={async (reason, description) => {
          if (reportingReview) {
            try {
              ...
              // Show some success feedback if needed, handled by UI gracefully
            } catch (err) {
