...
function generateHotels(city: string) {
  ...
  return hotelTemplates.map((h, index) => {
    ...
    // Vacancies between 0 (Sold out / Full) and 8
    ...
    // Unsplash featured query url to match the theme
...
function generateFlights(from: string, to: string, _date: string) {
  const airlines = [
...
function generateTrains(from: string, to: string, _date: string) {
...
interface TripBookingPortalProps {
  tripId: number
...
function TripBookingPortal({ tripId, destinationId, destinationName, tripStartDate }: TripBookingPortalProps) {
  ...
  type HotelData = ReturnType<typeof generateHotels>[0]
  
  // Query server hotels
  const { data: serverHotels } = useQuery<HotelData[]>({
    queryKey: ['hotels', destinationId],
    queryFn: () => destinationApi.getHotels(destinationId) as Promise<HotelData[]>,
